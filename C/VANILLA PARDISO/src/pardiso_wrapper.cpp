...
// Debugging and recheck procedures.
	...
	if (error != 0)
	{
		printf("ERROR: Matrix consistency: %d.\n", error);
		exit(1);
	}

	// Check vector RHS.
	pardiso_chkvec(&n, &nrhs, f, &error);
	if (error != 0)
	{
		printf("ERROR: RHS vector: %d.\n", error);
		exit(1);
	}

	// Matrix statistics.
	pardiso_printstats(&mtype, &n, A.a, A.ia, A.ja, &nrhs, f, &error);
	if (error != 0)
	{
		printf("ERROR: In statistics: %d.\n", error);
	}
	printf("PARDISO: DEBUG done.\n");
#endif
