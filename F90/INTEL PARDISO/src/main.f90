PROGRAM ELLSOLVE_F90
	...
	INTEGER(KIND=C_INT) :: PERM_USE = 0, PRECOND_USE = 0
	! VARIOUS TIMERS.
	...
	PRINT *, 'ELLSOLVEF: Solving with permutation.'
	PERM_USE = 1
	CALL CPU_TIME(START_TIME(3))
	CALL FLAT_LAPLACIAN(U, F, RES, S, %VAL(UINF), %VAL(ROBIN), %VAL(RSYM), %VAL(ZSYM),&
		%VAL(NRINTERIOR), %VAL(NZINTERIOR), %VAL(GHOST),&
		%VAL(DR), %VAL(DZ), %VAL(NORDER),&
		%VAL(PERM_USE), %VAL(PRECOND_USE))
	CALL CPU_TIME(END_TIME(3))
	TIMES(3) = END_TIME(3) - START_TIME(3)
	...
	PRINT *, 'ELLSOLVEF: Solving with CGS and permutation.'
	PRECOND_USE = 6
	PERM_USE = 1
	CALL CPU_TIME(START_TIME(5))
	CALL FLAT_LAPLACIAN(U, F, RES, S, %VAL(UINF), %VAL(ROBIN), %VAL(RSYM), %VAL(ZSYM),&
		%VAL(NRINTERIOR), %VAL(NZINTERIOR), %VAL(GHOST),&
		%VAL(DR), %VAL(DZ), %VAL(NORDER),&
		%VAL(PERM_USE), %VAL(PRECOND_USE))
	CALL CPU_TIME(END_TIME(5))
	TIMES(5) = END_TIME(5) - START_TIME(5)

	! PRINT EXECUTION TIMES.
	PRINT *, 'ELLSOLVEF: Normal solver took ', TIMES(1), ' seconds.'
	PRINT *, 'ELLSOLVEF: Permutation calculation took ', TIMES(2), ' seconds.'
	PRINT *, 'ELLSOLVEF: Solver with permutation took ', TIMES(3), ' seconds.'
	PRINT *, 'ELLSOLVEF: Solver with CGS took ', TIMES(4), ' seconds.'
	PRINT *, 'ELLSOLVEF: Solver with CGS and permutation took ', TIMES(5), ' seconds.'
