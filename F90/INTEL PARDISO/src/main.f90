PROGRAM ELLSOLVE_F90
    ...
    IF (SOLVER == 'flat') THEN
        ...
        CALL NDIFF_FLAT_LAPLACIAN(NDIFF, %VAL(NRINTERIOR), %VAL(NZINTERIOR))
        PRINT *, 'ELLSOLVEF: Number of differing elements in low rank update are ', NDIFF
        ! ALLOCATE DIFF ARRAY.
        CALL LOW_RANK_ALLOCATE(%VAL(NDIFF))
        ! FILL DIFF ARRAY FOR FLAT LAPLACIAN.
        CALL LOW_RANK_FLAT_LAPLACIAN(%VAL(NRINTERIOR), %VAL(NZINTERIOR))
    ...
    ELSE IF (SOLVER == 'general') THEN
        ...
        ! LOW RANK UPDATE SOLVE.
        ! GET NUMBER OF DIFFERING ELEMENTS.
        CALL NDIFF_GENERAL_ELLIPTIC(NDIFF, %VAL(NRINTERIOR), %VAL(NZINTERIOR), %VAL(NORDER))
        PRINT *, 'ELLSOLVEF: Number of differing elements in low rank update are ', NDIFF
        ! ALLOCATE DIFF ARRAY.
        CALL LOW_RANK_ALLOCATE(%VAL(NDIFF))
        ! FILL DIFF ARRAY FOR FLAT LAPLACIAN.
        CALL LOW_RANK_GENERAL_ELLIPTIC(%VAL(NRINTERIOR), %VAL(NZINTERIOR), %VAL(NORDER))
