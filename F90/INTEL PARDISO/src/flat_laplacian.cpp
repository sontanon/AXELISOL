	...
	const int precond_use) 	// Calculate and/or use preconditioner.
{
	...
	pardiso_wrapper(A, g_u, g_f, g_res, tol, &norm, &convergence, INFNORM, perm_use, precond_use);

#ifdef DEBUG
	write_single_file(g_u, "g_u.asc", NrTotal, NzTotal);
	write_single_file(g_f, "g_f.asc", NrTotal, NzTotal);
	write_single_file(g_res, "g_res.asc", NrTotal, NzTotal);
#endif
